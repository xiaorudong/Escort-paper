...
# dir.create("PLOTS/Script_realGold_pancreatic")
...
load("RDATA/step0_ss_realGold_pancreatic.RData")
load("RDATA/step1_realGold_pancreatic.RData")
...
DRLvsC6 <- LD_DCClusterscheck(dist_mat=dist(TSNE6_eval$Embedding, method = "euclidean"), DRdims=TSNE6_eval$Embedding, connectedCells = 2)
...
pdf("PLOTS/Script_realGold_pancreatic/gof_ss_evaluation.pdf", onefile = TRUE, width = 8, height = 4.5)
par(mfrow=c(1, 2))
gof_eval1 <- GOFeval(MDS1_eval)
gof_eval2 <- GOFeval(MDS2_eval, alpha = 0.17)
gof_eval3 <- GOFeval(MDS3_eval)
gof_eval4 <- GOFeval(TSNE4_eval)
gof_eval5 <- GOFeval(TSNE5_eval)
gof_eval6 <- GOFeval(TSNE6_eval)
gof_eval7 <- GOFeval(UMAP7_eval, alpha = 3.5)
...
pdf("PLOTS/Script_realGold_pancreatic/ushape_ss_evaluation.pdf", onefile = TRUE, width = 8, height = 4.5)
par(mfrow=c(1, 2))
...
save(scoredf, final_df, file = "RDATA/res_realGold_pancreatic.RData")
save.image(file = "RDATA/allres_realGold_pancreatic.RData")
