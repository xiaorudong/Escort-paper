...
scdata <- readRDS("DATA/sc_benchmark_real/psc-astrocyte-maturation-glia_sloan.rds")
...
scdata <- subset(scdata, subset = nFeature_RNA > 200 & nFeature_RNA < 2500)
...
save(norm_counts, rawcounts, scdata_time, 
     file = "RDATA/Manuscript/clean_realGold_astrocyte.RData")
...
save(embeddings, file = "RDATA/Manuscript/dr_realGold_astrocyte.RData")
...
# from Slingshot:
load("RDATA/Manuscript/slingshot_realGold_astrocyte.RData")
ushap_ss_evals <- mclapply(resobjs, UshapeDetector)
lapply(ushap_ss_evals, function(x) x$Ambpct)


# from Monocle3:
load("RDATA/Manuscript/Monocle3_realGold_astrocyte.RData")
