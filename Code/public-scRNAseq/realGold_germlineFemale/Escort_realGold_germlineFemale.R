...
scdata <- readRDS("DATA/sc_benchmark_real/germline-human-female-weeks_li.rds")
...
scdata <- subset(scdata, subset = nFeature_RNA < 3500 & nCount_RNA < 300000)
...
save(norm_counts, rawcounts, scdata_time, 
     file = "RDATA/Manuscript/clean_realGold_germlineFemale.RData")
...
save(embeddings, file = "RDATA/Manuscript/dr_realGold_germlineFemale.RData")
...
# from Slingshot:
load("RDATA/Manuscript/slingshot_realGold_germlineFemale.RData")
ushap_ss_evals <- mclapply(resobjs, UshapeDetector)
lapply(ushap_ss_evals, function(x) x$Ambpct)


# from Monocle3:
load("RDATA/Manuscript/Monocle3_realGold_germlineFemale.RData")
