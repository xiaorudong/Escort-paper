...
load("RDATA/example_realData_bone/orig_startcs_afterQC.RData")
...
cds <- preprocess_cds(cds, num_dim = 100)
cds <- reduce_dimension(cds, reduction_method = "UMAP")
reducedDims(cds)@listData[["UMAP"]] <- dimred10
cds <- cluster_cells(cds, reduction_method = "UMAP")
cds@clusters@listData[["UMAP"]][["clusters"]] <- list.cluster
plot_cells(cds, reduction_method = "UMAP", color_cells_by = "cluster")
cds <- learn_graph(cds, use_partition=F)
...
ti_out10 <- cds
dimred10 <- as.data.frame(umap_dim)
rawpse10 <- pt
pse10 <- pse
...
cds <- preprocess_cds(cds, num_dim = 100)
cds <- reduce_dimension(cds, reduction_method = "UMAP")
reducedDims(cds)@listData[["UMAP"]] <- dimred11
cds <- cluster_cells(cds, reduction_method = "UMAP")
cds@clusters@listData[["UMAP"]][["clusters"]] <- list.cluster
plot_cells(cds, reduction_method = "UMAP", color_cells_by = "cluster")
cds <- learn_graph(cds, use_partition=F)
...
ti_out11 <- cds
dimred11 <- as.data.frame(umap_dim)
rawpse11 <- pt
pse11 <- pse
...
cds <- preprocess_cds(cds, num_dim = 100)
cds <- reduce_dimension(cds, reduction_method = "UMAP")
reducedDims(cds)@listData[["UMAP"]] <- dimred12
cds <- cluster_cells(cds, reduction_method = "UMAP")
cds@clusters@listData[["UMAP"]][["clusters"]] <- list.cluster
plot_cells(cds, reduction_method = "UMAP", color_cells_by = "cluster")
cds <- learn_graph(cds, use_partition=F)
...
ti_out12 <- cds
dimred12 <- as.data.frame(umap_dim)
rawpse12 <- pt
pse12 <- pse
...
M3_obj10 <- info_traj(dimred10, rawpse = rawpse10, fitLine = fitLine10)
M3_obj11 <- info_traj(dimred11, rawpse = rawpse11, fitLine = fitLine11)
M3_obj12 <- info_traj(dimred12, rawpse = rawpse12, fitLine = fitLine12)
...
PCA10_eval <- TEvalobj(dimred10, traj = M3_obj10)
PCA11_eval <- TEvalobj(dimred11, traj = M3_obj11)
PCA12_eval <- TEvalobj(dimred12, traj = M3_obj12)
...
save(MDS1_eval, MDS2_eval, MDS3_eval,
     ...
     file = "RDATA/example_realData_bone/step0_m3_cls_afterQC.RData")
