...
load("RDATA/example_realData_bone/step0_ss_cls_afterQC.RData")
...
DRLvsC10 <- LD_DCClusterscheck(dist_mat=dist(PCA10_eval$Embedding, method = "euclidean"), DRdims=PCA10_eval$Embedding, connectedCells = 1)
plot(PCA10_eval$Embedding, col=alpha(DRLvsC10$Clusters,0.5), pch=16, main=deparse(substitute(PCA10_eval)))
legend("topleft", legend=as.character(1:DRLvsC10$K), col=c(1:DRLvsC10$K), pch=16, cex = 0.5)
DRLvsC10$DCcheck
DRLvsC10$clusterLocation

DRLvsC11 <- LD_DCClusterscheck(dist_mat=dist(PCA11_eval$Embedding, method = "euclidean"), DRdims=PCA11_eval$Embedding, connectedCells = 1)
plot(PCA11_eval$Embedding, col=alpha(DRLvsC11$Clusters,0.5), pch=16, main=deparse(substitute(PCA11_eval)))
legend("topleft", legend=as.character(1:DRLvsC11$K), col=c(1:DRLvsC11$K), pch=16, cex = 0.5)
DRLvsC11$DCcheck
DRLvsC11$clusterLocation

DRLvsC12 <- LD_DCClusterscheck(dist_mat=dist(PCA12_eval$Embedding, method = "euclidean"), DRdims=PCA12_eval$Embedding, connectedCells = 1)
plot(PCA12_eval$Embedding, col=alpha(DRLvsC12$Clusters,0.5), pch=16, main=deparse(substitute(PCA12_eval)))
legend("topleft", legend=as.character(1:DRLvsC12$K), col=c(1:DRLvsC12$K), pch=16, cex = 0.5)
DRLvsC12$DCcheck
DRLvsC12$clusterLocation

DRLvsCorig <- LD_DCClusterscheck(dist_mat=dist(ss_orig_eval$Embedding, method = "euclidean"), DRdims=ss_orig_eval$Embedding, connectedCells = 1)
plot(ss_orig_eval$Embedding, col=alpha(DRLvsCorig$Clusters,0.5), pch=16, main=deparse(substitute(ss_orig_eval)))
legend("topleft", legend=as.character(1:DRLvsCorig$K), col=c(1:DRLvsCorig$K), pch=16, cex = 0.5)
DRLvsCorig$DCcheck
DRLvsCorig$clusterLocation
...
simi_cells <- Similaritycheck(norm_counts=norm_counts, obj=PCA10_eval, Cluters=LvsC) # we use all norm counts instead of counts data after feature selection
simi_cells$SimilarityLevel
(rate_good <- sum(simi_cells$SimilarityLevel[as.numeric(names(simi_cells$SimilarityLevel))<=1])/ncol(norm_counts))
simiRetain <- c(simiRetain, rate_good)
simi_cells10 <- simi_cells

simi_cells <- Similaritycheck(norm_counts=norm_counts, obj=PCA11_eval, Cluters=LvsC) # we use all norm counts instead of counts data after feature selection
simi_cells$SimilarityLevel
(rate_good <- sum(simi_cells$SimilarityLevel[as.numeric(names(simi_cells$SimilarityLevel))<=1])/ncol(norm_counts))
simiRetain <- c(simiRetain, rate_good)
simi_cells11 <- simi_cells

simi_cells <- Similaritycheck(norm_counts=norm_counts, obj=PCA12_eval, Cluters=LvsC) # we use all norm counts instead of counts data after feature selection
simi_cells$SimilarityLevel
(rate_good <- sum(simi_cells$SimilarityLevel[as.numeric(names(simi_cells$SimilarityLevel))<=1])/ncol(norm_counts))
simiRetain <- c(simiRetain, rate_good)
simi_cells12 <- simi_cells

simi_cells <- Similaritycheck(norm_counts=norm_counts, obj=ss_orig_eval, Cluters=LvsC) # we use all norm counts instead of counts data after feature selection
simi_cells$SimilarityLevel
(rate_good <- sum(simi_cells$SimilarityLevel[as.numeric(names(simi_cells$SimilarityLevel))<=1])/ncol(norm_counts))
simiRetain <- c(simiRetain, rate_good)
simi_cellsorig <- simi_cells
...
ifConn <- c(DRLvsC1$ifConnected, DRLvsC2$ifConnected, DRLvsC3$ifConnected,
            ...
            DRLvsC10$ifConnected, DRLvsC11$ifConnected, DRLvsC12$ifConnected, 
            DRLvsCorig$ifConnected)
...
pdf("PLOTS/example_realData_bone/gof_ss_evaluation_afterQC.pdf", onefile = TRUE, width = 8, height = 4.5)
par(mfrow=c(1, 2))
...
gof_eval10 <- GOFeval(PCA10_eval)
gof_eval11 <- GOFeval(PCA11_eval)
gof_eval12 <- GOFeval(PCA12_eval)
gof_evalorig <- GOFeval(ss_orig_eval)
dev.off()
...
scoredf$GOF <- c(gof_eval1$occupiedRate, gof_eval2$occupiedRate, gof_eval3$occupiedRate, 
                 ...
                 gof_eval10$occupiedRate, gof_eval11$occupiedRate, gof_eval12$occupiedRate,
                 gof_evalorig$occupiedRate) 
...
pdf("PLOTS/example_realData_bone/ushape_ss_cls_evaluation_afterQC.pdf", onefile = TRUE, width = 8, height = 4.5)
par(mfrow=c(1, 2))
...
ushap_eval10 <- UshapeDetector(PCA10_eval)
ushap_eval11 <- UshapeDetector(PCA11_eval)
ushap_eval12 <- UshapeDetector(PCA12_eval)
ushap_evalorig <- UshapeDetector(ss_orig_eval)
dev.off()
...
save(scoredf, final_df, file = "RDATA/example_realData_bone/res_cls_afterQC.RData")
save.image(file = "RDATA/example_realData_bone/allres_cls_afterQC.RData")
