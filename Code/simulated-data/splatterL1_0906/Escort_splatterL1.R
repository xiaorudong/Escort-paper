...
scdata <- readRDS("DATA/sc_benchmark/synthetic_splatter_linear_1.rds")
...
save(norm_counts, rawcounts, scdata_time, 
     file = "RDATA/Manuscript/clean_splatter_L1.RData")
...
# Test Homogeneous
cor_test <- step1_testHomogeneous(normcounts=norm_counts, g = 50)
cor_test$decision
cor_test$signal_pct
...
save(embeddings, file = "RDATA/Manuscript/dr_splatter_L1.RData")
...
# from Slingshot:
load("RDATA/Manuscript/slingshot_splatterL1.RData")
ushap_ss_evals <- mclapply(resobjs, UshapeDetector)
lapply(ushap_ss_evals, function(x) x$Ambpct)


# from Monocle3:
load("RDATA/Manuscript/Monocle3_splatterL1.RData")
