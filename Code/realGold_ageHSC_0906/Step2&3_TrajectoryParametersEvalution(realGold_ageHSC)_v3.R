...
# dir.create("PLOTS/Script_removeCC")
...
load("RDATA/step0_ss_removeCC.RData")
load("RDATA/step1_removeCC.RData")
...
simi_cells <- Similaritycheck(norm_counts=correct_counts, obj=MDS1_eval, Cluters=LvsC) # we use all norm counts instead of counts data after feature selection
...
simi_cells <- Similaritycheck(norm_counts=correct_counts, obj=MDS2_eval, Cluters=LvsC) # we use all norm counts instead of counts data after feature selection
simi_cells$SimilarityLevel
(rate_good <- sum(simi_cells$SimilarityLevel[as.numeric(names(simi_cells$SimilarityLevel))<=1])/ncol(norm_counts))
simiRetain <- c(simiRetain, rate_good)
simi_cells2 <- simi_cells

simi_cells <- Similaritycheck(norm_counts=correct_counts, obj=MDS3_eval, Cluters=LvsC) # we use all norm counts instead of counts data after feature selection
simi_cells$SimilarityLevel
(rate_good <- sum(simi_cells$SimilarityLevel[as.numeric(names(simi_cells$SimilarityLevel))<=1])/ncol(norm_counts))
simiRetain <- c(simiRetain, rate_good)
simi_cells3 <- simi_cells

simi_cells <- Similaritycheck(norm_counts=correct_counts, obj=TSNE4_eval, Cluters=LvsC) # we use all norm counts instead of counts data after feature selection
simi_cells$SimilarityLevel
(rate_good <- sum(simi_cells$SimilarityLevel[as.numeric(names(simi_cells$SimilarityLevel))<=1])/ncol(norm_counts))
simiRetain <- c(simiRetain, rate_good)
simi_cells4 <- simi_cells

simi_cells <- Similaritycheck(norm_counts=correct_counts, obj=TSNE5_eval, Cluters=LvsC) # we use all norm counts instead of counts data after feature selection
simi_cells$SimilarityLevel
(rate_good <- sum(simi_cells$SimilarityLevel[as.numeric(names(simi_cells$SimilarityLevel))<=1])/ncol(norm_counts))
simiRetain <- c(simiRetain, rate_good)
simi_cells5 <- simi_cells

simi_cells <- Similaritycheck(norm_counts=correct_counts, obj=TSNE6_eval, Cluters=LvsC) # we use all norm counts instead of counts data after feature selection
simi_cells$SimilarityLevel
(rate_good <- sum(simi_cells$SimilarityLevel[as.numeric(names(simi_cells$SimilarityLevel))<=1])/ncol(norm_counts))
simiRetain <- c(simiRetain, rate_good)
simi_cells6 <- simi_cells

simi_cells <- Similaritycheck(norm_counts=correct_counts, obj=UMAP7_eval, Cluters=LvsC) # we use all norm counts instead of counts data after feature selection
simi_cells$SimilarityLevel
(rate_good <- sum(simi_cells$SimilarityLevel[as.numeric(names(simi_cells$SimilarityLevel))<=1])/ncol(norm_counts))
simiRetain <- c(simiRetain, rate_good)
simi_cells7 <- simi_cells

simi_cells <- Similaritycheck(norm_counts=correct_counts, obj=UMAP8_eval, Cluters=LvsC) # we use all norm counts instead of counts data after feature selection
simi_cells$SimilarityLevel
(rate_good <- sum(simi_cells$SimilarityLevel[as.numeric(names(simi_cells$SimilarityLevel))<=1])/ncol(norm_counts))
simiRetain <- c(simiRetain, rate_good)
simi_cells8 <- simi_cells

simi_cells <- Similaritycheck(norm_counts=correct_counts, obj=UMAP9_eval, Cluters=LvsC) # we use all norm counts instead of counts data after feature selection
simi_cells$SimilarityLevel
(rate_good <- sum(simi_cells$SimilarityLevel[as.numeric(names(simi_cells$SimilarityLevel))<=1])/ncol(norm_counts))
simiRetain <- c(simiRetain, rate_good)
...
pdf("PLOTS/Script_removeCC/gof_ss_evaluation.pdf", onefile = TRUE, width = 8, height = 4.5)
par(mfrow=c(1, 2))
...
pdf("PLOTS/Script_removeCC/ushape_ss_evaluation.pdf", onefile = TRUE, width = 8, height = 4.5)
par(mfrow=c(1, 2))
...
save(scoredf, final_df, file = "RDATA/res_removeCC.RData")
save.image(file = "RDATA/allres_removeCC.RData")
