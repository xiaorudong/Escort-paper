...
UshapeDetector <- function(obj, dimred, PLOTname, outlierdetect='neutral') {
  ...
  for (i in 1:nlineage) {
    ...
    outdect0 <- boxB(x = knn_sd, k = 1.5, method="resistant")
    upperB0 <- outdect0$fences[2]
    
    outdect1 <- boxB(x = knn_sd, k = 1.5, method="adjbox")
    upperB1 <- outdect1$fences[2]
    
    outdect2 <- boxB(x = knn_sd, k = 1.5, method="asymmetric")
